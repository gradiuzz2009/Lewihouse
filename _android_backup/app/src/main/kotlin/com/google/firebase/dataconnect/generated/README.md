...
is the query named "ListElectricityMeters",
which can be accessed via the [ExampleConnector.listElectricityMeters] property.


An example of the property for a mutation
is the mutation named "CreateMaintenanceTicket",
which can be accessed via the [ExampleConnector.createMaintenanceTicket] property.
...
For example, the "ListElectricityMeters" query has no variables
and can be executed via the
[ExampleConnector.listElectricityMeters]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listElectricityMeters.execute()
println("ListElectricityMeters query returned: ${queryResult.data}")
```
...
For example, the "CreateMaintenanceTicket" mutation has 8 required variables ("roomNumber", "residentId", "residentName", "title", "category", "description", "priority", and "status")
and can be executed via the [ExampleConnector.createMaintenanceTicket]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createMaintenanceTicket.execute(roomNumber="waldo", residentId="fred", residentName="quux", title="thud", category="grault", description="baz", priority="foo", status="garply")
println("CreateMaintenanceTicket mutation returned: ${mutationResult.data}")
```
...
For example, the "CreateResident" mutation has 6 optional variables ("moveInDate", "leaseEndDate", "monthlyRent", "depositAmount", "emergencyContact", and "emergencyPhone")
and can be executed via the [ExampleConnector.createResident]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createResident.execute(fullName="grault", email="qux", phone="garply", roomNumber="waldo", status="garply") {
  moveInDate = "fred"
  leaseEndDate = "bar"
  monthlyRent = 7278.36
  depositAmount = 72.25
  emergencyContact = "garply"
  emergencyPhone = "waldo"
}
println("CreateResident mutation returned: ${mutationResult.data}")
```
